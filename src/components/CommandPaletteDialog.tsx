...
import { Search, Command, ArrowRight, MapPin, Rocket, type LucideIcon } from "lucide-react";
import Fuse from "fuse.js";
import { cn } from "@/lib/utils";
import { Input } from "@/components/ui/input";
import { COMMAND_ACTION_THEME, COMMAND_STATIC_ITEMS } from "@/data/navigation";
import { WEATHER_LOCATIONS } from "@/data/locations";
...
/**
 * Lazy-loaded dialog body (see `CommandPalette.tsx` trigger). Holds Fuse.js,
 * the item registry and all dialog UI — none of which is needed until first
 * open, so it stays out of the critical entry chunk.
 */
export function CommandPaletteDialog({ open, onOpenChange }: CommandPaletteDialogProps) {
  ...
  // 1. Define searchable items — single source in data/navigation.ts + weather + quick-start
  const ALL_ITEMS: CommandItem[] = useMemo(() => {
    const weatherItems: CommandItem[] = WEATHER_LOCATIONS.map((loc) => ({
      id: `weather-${loc.id}`,
      title: `Weather: ${loc.label}`,
      category: "Tools" as const,
      icon: MapPin,
      synonyms: [loc.country, loc.id, `${loc.lat}`, `${loc.lon}`, "weather", "forecast", "tanstack query"],
      action: () => {
        navigate({ to: "/", search: { location: loc.id } as never });
        setTimeout(() => {
    ...
    const quickStartItem: CommandItem = {
      id: "quick-start",
      title: "Quick Start — Clone Fast Orbit",
      category: "Sections" as const,
      icon: Rocket,
      synonyms: ["quick start", "clone", "install", "pnpm dev", "starter", "ssg"],
      to: "/#quick-start",
    };
  ...
  // 2. Initialize Fuse.js fuzzy search
  const fuse = useMemo(
    () =>
      new Fuse(ALL_ITEMS, {
        keys: [
  ...
  // 5. Autocomplete Ghost Suggestion
  ...
  // 6. Global Keydown Handler when Open (ArrowDown, ArrowUp, Enter, Tab)
  useEffect(() => {
    if (!open) return;
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (filteredItems[selectedIndex]) {
          handleSelectRef.current(filteredItems[selectedIndex]);
        }
      } else if (e.key === "Tab" && suggestion && filteredItems[0]) {
        e.preventDefault();
        e.stopPropagation();
  ...
  }, [open, filteredItems, selectedIndex, suggestion]);
      ...
      <BaseDialog.Portal>
        ...
        <BaseDialog.Popup
          ...
          <BaseDialog.Description className='sr-only'>
            Search Fast Orbit for tools, pages, settings, and actions.
          ...
          <div className='flex items-center justify-between border-t border-border bg-surface-1 px-4 py-2.5 text-xs text-muted-foreground'>
            ...
            <div className='flex items-center gap-1.5'>
              ...
              <span className='font-mono text-xs font-semibold tracking-wider'>FAST ORBIT</span>
            </div>
          </div>
        </BaseDialog.Popup>
      </BaseDialog.Portal>
