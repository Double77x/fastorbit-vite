# Graphite Coding Standards & Guidelines

This document outlines the strict technical standards and best practices for the Graphite project. All code contributions must adhere to these principles to ensure scalability, maintainability, and performance.
...
- **Tool:** **React 19 Native Hoisting**.
- **Hybrid Strategy:**
  - **Static Defaults (`index.html`):** MUST contain baseline "fail-safe" metadata (brand title, fallback description, viewport, theme-colors, and PWA tags).
...
- **Virtualization (`@tanstack/react-virtual`):**
  ...
  - **Never virtualize what virtualization can't fix:** TanStack Charts **marks** are rendered imperatively by the SVG `RendererChart` — virtualization does not apply. The correct perf fix for chart marks is the **Canvas renderer** (`@tanstack/charts/canvas` / `CanvasChart`) plus **downsampling** (e.g. `transform/bin-xy`), not `useVirtualizer`.
  - **Base UI Select vs Combobox:** **`Select` cannot be safely virtualized** with `@tanstack/react-virtual` — keyboard navigation breaks past the virtual window because Base UI Select builds its options array from rendered children (see mui/base-ui#2282). Use the plain Select for small lists. Only **`Combobox`** supports virtualization natively (`virtualized` prop + `items` + `filteredItems` + `onItemHighlighted`); use it only above a threshold (see `ColumnPicker.tsx`, `COMBOBOX_THRESHOLD = 150`).
  - **CLS / shift safety contract** (see `DataGrid.tsx`, `ColumnPicker.tsx`):
...
- **False Positive Awareness:**
  ...
  - **Example:** Refer to `ToolLayout.tsx` for the established standard on how to abstract complex, shared UI structures in this project.
...
- **Preloading:** Critical font weights (e.g., Regular 400 and Bold 700) **MUST** be preloaded in the `<head>` of `index.html` using `<link rel="preload">` to prevent FOUT (Flash of Unstyled Text).
